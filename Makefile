...
# Project variables.
PACKAGE := github.com/chainbing/node
...
# Docker configs.
DOCKER_IMAGE = node-build
DOCKER_BUILD_FILE = build/Dockerfile

# Project configs.
MODE ?= sync
CONFIG ?= $(GOBASE)/cmd/cbnode/cfg.buidler.toml
PGHOST ?= localhost
PGPORT ?= 4012
PGUSER ?= chainbing
PGPASSWORD ?= yourpasswordhere
PGDATABASE ?= chainbing
...
build: clean migration-pack
	echo "  >  Building Chainbing binary..."
...
## run-node: Run Chainbing node.
run-node: build
	echo "  >  Running $(PROJECT_NAME)"
	$(GOBIN)/$(GOBINARY) run --mode $(MODE) --cfg $(CONFIG)

## wipedbs: Clean Chainbing node databases.
wipedbs: build
...
run-database-container:
	...
	docker run --rm --name chainbing-l2db-$(PGPORT) -p $(PGPORT):5432 -e POSTGRES_DB=$(PGDATABASE) -e POSTGRES_USER=$(PGUSER) -e POSTGRES_PASSWORD="$(PGPASSWORD)" -d postgres

## stop-database-container: Stop the Postgres container
stop-database-container:
	echo "  >  Stopping the postgreSQL DB..."
	docker stop chainbing-l2db-$(PGPORT)

## exec: Run given command. e.g; make exec run="go test ./..."
exec:
	$(GOENVVARS) $(run)

## install: Install the cbnode cli as a service in coord mode
install:
	echo "  > Installing cbnode as a service"
	echo "  > Checking requirements"
ifneq ("$(wildcard ./dist/cbnode)","")
	echo "  - cbnode file found!"
else
	echo "  - cbnode file not found!"
	echo "  - please, run make build before make install!"
	test -f ./dist/cbnode
endif
ifneq ("$(wildcard ./cmd/cbnode/cfg.builder.toml)","")
	echo "  - config template found!"
else
	echo "  - config template not found!"
	echo "  - please, check the ./cmd/cbnode/cfg.builder.toml!"
	test -f ./cmd/cbnode/cfg.builder.toml
endif
ifneq ("$(wildcard /etc/chainbing/config.toml)","")
	echo "  > Config file already exists - ignored."
else
	echo "  > Copying config file to /etc/chainbing"
	mkdir -p /etc/chainbing
	cp cmd/cbnode/cfg.builder.toml /etc/chainbing/config.toml
endif
ifeq ("$(wildcard /etc/systemd/system/cbnode.service)", "")
	echo "  > Registering as a service"
	touch /etc/systemd/system/cbnode.service
	echo "[Unit]" | tee -a /etc/systemd/system/cbnode.service > /dev/null
	echo "Description=Chainbing Node" | tee -a /etc/systemd/system/cbnode.service > /dev/null
	echo "After=postgresql.service" | tee -a /etc/systemd/system/cbnode.service > /dev/null
	echo "StartLimitBurst=5" | tee -a /etc/systemd/system/cbnode.service > /dev/null
	echo "StartLimitIntervalSec=60" | tee -a /etc/systemd/system/cbnode.service > /dev/null
	echo "" | tee -a /etc/systemd/system/cbnode.service > /dev/null
	echo "[Service]" | tee -a /etc/systemd/system/cbnode.service > /dev/null
	echo "Type=simple" | tee -a /etc/systemd/system/cbnode.service > /dev/null
	echo "Restart=always" | tee -a /etc/systemd/system/cbnode.service > /dev/null
	echo "RestartSec=1" | tee -a /etc/systemd/system/cbnode.service > /dev/null
	echo "ExecStart=/usr/local/bin/cbnode run --mode coord --cfg /etc/chainbing/config.toml" | tee -a /etc/systemd/system/cbnode.service > /dev/null
	echo "KillMode=process" | tee -a /etc/systemd/system/cbnode.service > /dev/null
	echo "StandardOutput=append:/var/log/node.log" | tee -a /etc/systemd/system/cbnode.service > /dev/null
	echo "StandardError=append:/var/log/node.log" | tee -a /etc/systemd/system/cbnode.service > /dev/null
	echo "" | tee -a /etc/systemd/system/cbnode.service > /dev/null
	echo "[Install]" | tee -a /etc/systemd/system/cbnode.service > /dev/null
	echo "WantedBy=multi-user.target" | tee -a /etc/systemd/system/cbnode.service > /dev/null
	echo "" | tee -a /etc/systemd/system/cbnode.service > /dev/null
	systemctl daemon-reload
else
	echo "  > Service is already registered. Will be stoped!"
	service cbnode stop
	rm /usr/local/bin/cbnode
endif
	echo "  > Copying cb binary to /usr/local/bin"
	cp dist/cbnode /usr/local/bin/cbnode
	echo "  > Service is ready. Please update the configs at /etc/chainbing/config.toml"
	echo "  > You can use the service with service cbnode status|start|stop"
	echo "  Bye."
