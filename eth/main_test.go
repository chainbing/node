...
var (
	...
	tokenCBAddressConst     ethCommon.Address
	chainbingRollupAddressConst ethCommon.Address
	wdelayerAddressConst     ethCommon.Address
	wdelayerTestAddressConst ethCommon.Address
	tokenCB                 ethCommon.Address
...
var (
	...
	chainbingRollupTestAccount      *accounts.Account
	chainbingRollupTestAddressConst ethCommon.Address
)
...
var (
	...
	ethereumClientChainbing           *EthereumClient
)
...
func getEnvVariables() {
	...
	var tokenCBAddressStr = os.Getenv("TOKENCB")
	var chainbingRollupAddressStr = os.Getenv("HERMEZ")
	...
	if auctionAddressStr == "" || auctionTestAddressStr == "" || tokenCBAddressStr == "" ||
		chainbingRollupAddressStr == "" || wdelayerAddressStr == "" || wdelayerTestAddressStr == "" ||
		genesisBlockEnv == "" {
		log.Fatal(errEnvVar)
	}
	...
	tokenCBAddressConst = ethCommon.HexToAddress(tokenCBAddressStr)
	chainbingRollupAddressConst = ethCommon.HexToAddress(chainbingRollupAddressStr)
...
func TestMain(m *testing.M) {
	...
	if os.Getenv("INTEGRATION") != "" {
		...
		chainbingRollupTestAccount, chainbingRollupTestAddressConst = genAcc(w, ks, 4)
		...
		auctionClient, err = NewAuctionClient(ethereumClientGov, auctionAddressConst, tokenCB)
		if err != nil {
			log.Fatal(err)
		}
		auctionClientTest, err = NewAuctionClient(ethereumClientGov, auctionTestAddressConst, tokenCB)
		if err != nil {
			log.Fatal(err)
		}
		rollupClient, err = NewRollupClient(ethereumClientGov, chainbingRollupAddressConst)
		if err != nil {
			log.Fatal(err)
		}
		...
		ethereumClientChainbing, err = NewEthereumClient(ethClient, chainbingRollupTestAccount, ks, nil)
		if err != nil {
			log.Fatal(err)
		}
