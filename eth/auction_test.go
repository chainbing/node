...
func TestAuctionConstants(t *testing.T) {
	...
	assert.Equal(t, auctionConstants.ChainbingRollup, chainbingRollupTestAddressConst)
	assert.Equal(t, auctionConstants.InitialMinimalBidding, INITMINBID)
	assert.Equal(t, auctionConstants.TokenCB, tokenCBAddressConst)
...
func TestAuctionGetClaimableCB(t *testing.T) {
	bidderAddress := governanceAddressConst

	claimableCB, err := auctionClientTest.AuctionGetClaimableCB(bidderAddress)
	require.Nil(t, err)
	assert.Equal(t, claimableCB.Int64(), int64(0))
}
...
func TestAuctionGetClaimableCB2(t *testing.T) {
	bidderAddress := governanceAddressConst
	amount := new(big.Int)
	...
	claimableCB, err := auctionClientTest.AuctionGetClaimableCB(bidderAddress)
	require.Nil(t, err)
	assert.Equal(t, claimableCB, amount)
}

func TestAuctionClaimCB(t *testing.T) {
	amount := new(big.Int)
	amount.SetString("11000000000000000000", 10)

	_, err := auctionClientTest.AuctionClaimCB()
	require.Nil(t, err)
	currentBlockNum, err := auctionClientTest.client.EthLastBlock()
	require.Nil(t, err)
	auctionEvents, err := auctionClientTest.AuctionEventsByBlock(currentBlockNum, nil)
	require.Nil(t, err)
	assert.Equal(t, amount, auctionEvents.CBClaimed[0].Amount)
	assert.Equal(t, governanceAddressConst, auctionEvents.CBClaimed[0].Owner)
}

func TestAuctionForge(t *testing.T) {
	auctionClientTestChainbing, err := NewAuctionClient(ethereumClientChainbing,
		auctionTestAddressConst, tokenCB)
	require.Nil(t, err)
	slotConst := 4
	blockNum := int64(int(blocksPerSlot)*slotConst + int(genesisBlock))
	currentBlockNum, err := auctionClientTestChainbing.client.EthLastBlock()
	require.Nil(t, err)
	...
	_, err = auctionClientTestChainbing.AuctionForge(governanceAddressConst)
	require.Nil(t, err)
}
