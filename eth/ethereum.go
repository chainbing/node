...
import (
	...
	"github.com/chainbing/node/eth/contracts/tokencb"
...
func (c *EthereumClient) EthERC20Consts(tokenAddress ethCommon.Address) (*ERC20Consts, error) {
	// We use the CB token smart contract interfacehere because it's an
	// ERC20, which allows us to access the standard ERC20 constants.
	instance, err := tokencb.NewTokencb(tokenAddress, c.client)
	if err != nil {
		return nil, tracerr.Wrap(err)
	}
...
// in ganache: https://github.com/chainbing/node/issues/317
