...
import (
	...
	auction "github.com/chainbing/node/eth/contracts/auction"
...
// AuctionEventInitialize is the InitializeChainbingAuctionProtocolEvent event of
// the Smart Contract
...
func NewAuctionEvents() AuctionEvents {
	return AuctionEvents{
		...
		CBClaimed:            make([]AuctionEventCBClaimed, 0),
	}
}
...
func (c *AuctionClient) AuctionGetClaimableCB(
	...
	if err := c.client.Call(func(ec *ethclient.Client) error {
		claimableCB, err = c.auction.GetClaimableCB(c.opts, claimAddress)
		return tracerr.Wrap(err)
	}); err != nil {
		return nil, tracerr.Wrap(err)
	}
	return claimableCB, nil
}
...
func (c *AuctionClient) AuctionBid(amount *big.Int, slot int64, bidAmount *big.Int,
	...
	if tx, err = c.client.CallAuth(
		...
		func(ec *ethclient.Client, auth *bind.TransactOpts) (*types.Transaction, error) {
			...
			spender := c.address
			nonce, err := c.token.cb.Nonces(c.opts, owner)
			if err != nil {
				return nil, tracerr.Wrap(err)
			}
...
func (c *AuctionClient) AuctionClaimCB() (tx *types.Transaction, err error) {
	if tx, err = c.client.CallAuth(
		0,
		func(ec *ethclient.Client, auth *bind.TransactOpts) (*types.Transaction, error) {
			return c.auction.ClaimCB(auth)
		},
	); err != nil {
		return nil, tracerr.Wrap(fmt.Errorf("Failed claim CB: %w", err))
	}
	return tx, nil
}
...
func (c *AuctionClient) AuctionConstants() (auctionConstants *common.AuctionConstants, err error) {
	...
	if err := c.client.Call(func(ec *ethclient.Client) error {
		...
		auctionConstants.ChainbingRollup, err = c.auction.ChainbingRollup(c.opts)
		if err != nil {
			return tracerr.Wrap(err)
		}
		...
		auctionConstants.TokenCB, err = c.auction.TokenCB(c.opts)
		if err != nil {
			return tracerr.Wrap(err)
		}
		return nil
...
var (
	...
	logAuctionCBClaimed = crypto.Keccak256Hash([]byte(
		"CBClaimed(address,uint128)"))
	logAuctionInitialize = crypto.Keccak256Hash([]byte(
		"InitializeChainbingAuctionProtocolEvent(address,address,string," +
...
func (c *AuctionClient) AuctionEventInit(genesisBlockNum int64) (*AuctionEventInitialize, int64, error) {
	...
	if len(logs) != 1 {
		return nil, 0,
			tracerr.Wrap(fmt.Errorf("no event of type InitializeChainbingAuctionProtocolEvent found"))
	}
	vLog := logs[0]
	if vLog.Topics[0] != logAuctionInitialize {
		return nil, 0, tracerr.Wrap(fmt.Errorf("event is not InitializeChainbingAuctionProtocolEvent"))
	}

	var auctionInit AuctionEventInitialize
	if err := c.contractAbi.UnpackIntoInterface(&auctionInit,
		"InitializeChainbingAuctionProtocolEvent", vLog.Data); err != nil {
		return nil, 0, tracerr.Wrap(err)
	}
...
func (c *AuctionClient) AuctionEventsByBlock(blockNum int64,
	...
	for _, vLog := range logs {
		...
		case logAuctionCBClaimed:
			var CBClaimed AuctionEventCBClaimed
			if err := c.contractAbi.UnpackIntoInterface(&CBClaimed, "CBClaimed",
				vLog.Data); err != nil {
				return nil, tracerr.Wrap(err)
			}
			CBClaimed.Owner = ethCommon.BytesToAddress(vLog.Topics[1].Bytes())
			auctionEvents.CBClaimed = append(auctionEvents.CBClaimed, CBClaimed)
		}
	}
