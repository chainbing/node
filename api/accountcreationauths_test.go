...
type testAuth struct {
	EthAddr   string    `json:"cbEthereumAddress" binding:"required"`
...
func genTestAuths(auths []*common.AccountCreationAuth) []testAuth {
	...
	for _, auth := range auths {
		testAuths = append(testAuths, testAuth{
			EthAddr:   common.EthAddrToCb(auth.EthAddr),
...
func TestAccountCreationAuth(t *testing.T) {
	...
	// Wrong addr
	badAuth := tc.auths[0]
	badAuth.EthAddr = common.EthAddrToCb(ethCommon.BigToAddress(big.NewInt(1)))
	...
	// GET
	// 400
	err = doBadReq("GET", endpoint+"cb:0xFooBar", nil, 400)
	assert.NoError(t, err)
	// 404
	err = doBadReq("GET", endpoint+"cb:0x0000000000000000000000000000000000000001", nil, 404)
	assert.NoError(t, err)
}
