...
func ExitsFiltersStructValidation(sl validator.StructLevel) {
	...
	if ef.Addr != "" && ef.Bjj != "" {
		sl.ReportError(ef.Addr, "cbEthereumAddress", "Addr", "cbethaddrorbjj", "")
		sl.ReportError(ef.Bjj, "BJJ", "Bjj", "cbethaddrorbjj", "")
	}
	...
	if ef.AccountIndex != "" && (ef.Addr != "" || ef.Bjj != "" || ef.TokenID != nil) {
		...
		sl.ReportError(ef.Addr, "cbEthereumAddress", "Addr", "onlyaccountindex", "")
...
func ParseExitsFilters(c *gin.Context, v *validator.Validate) (historydb.GetExitsAPIRequest, error) {
	...
	addr, err := common.CbStringToEthAddr(exitsFilters.Addr, "cbEthereumAddress")
	if err != nil {
		return historydb.GetExitsAPIRequest{}, tracerr.Wrap(err)
	}

	bjj, err := common.CbStringToBJJ(exitsFilters.Bjj, "BJJ")
	if err != nil {
		return historydb.GetExitsAPIRequest{}, tracerr.Wrap(err)
	}
