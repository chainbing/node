...
func (a *API) postAccountCreationAuth(c *gin.Context) {
	...
	isValid, err := commonAuth.VerifySignature(a.cg.ChainID, a.chainbingAddress)
	if !isValid && err != nil {
		retBadReq(&apiError{
...
type receivedAuth struct {
	EthAddr   apitypes.StrCbEthAddr `json:"cbEthereumAddress" binding:"required"`
	BJJ       apitypes.StrCbBJJ     `json:"bjj" binding:"required"`
