...
func genTestExits(
	...
	for _, exit := range commonExits {
		...
		allExits = append(allExits, testExit{
			...
			AccountIdx: common.IdxToCb(exit.AccountIdx, token.Symbol),
			BJJ:        apitypes.NewCbBJJ(acc.BJJ),
			EthAddr:    apitypes.NewCbEthAddr(acc.EthAddr),
			MerkleProof: testCVP{
...
func TestGetExits(t *testing.T) {
	...
	// 400
	path = fmt.Sprintf(
		"%s?accountIndex=%s&cbEthereumAddress=%s",
...
func TestGetExit(t *testing.T) {
	...
	assert.NoError(t, err)
	err = doBadReq("GET", endpoint+"-1/cb:BOOM:1", nil, 400)
	assert.NoError(t, err)
	// 404
	err = doBadReq("GET", endpoint+"494/cb:XXX:1", nil, 404)
	assert.NoError(t, err)
}
