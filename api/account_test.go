...
type testAccount struct {
	...
	Idx       apitypes.CbIdx        `json:"accountIndex"`
	BatchNum  common.BatchNum        `json:"batchNum"`
	PublicKey apitypes.CbBJJ        `json:"bjj"`
...
func genTestAccounts(accounts []common.Account, tokens []historydb.TokenWithUSD) []testAccount {
	...
	for x, account := range accounts {
		...
		tAccount := testAccount{
			ItemID:    uint64(x + 1),
			Idx:       apitypes.CbIdx(common.IdxToCb(account.Idx, token.Symbol)),
			PublicKey: apitypes.NewCbBJJ(account.BJJ),
			EthAddr:   apitypes.NewCbEthAddr(account.EthAddr),
			Nonce:     account.Nonce,
...
func TestGetAccounts(t *testing.T) {
	...
	fetchedAccounts = []testAccount{}
	// Filter by ethAddr
	path = fmt.Sprintf("%s?cbEthereumAddress=%s&limit=%d", endpoint, tc.accounts[3].EthAddr, limit)
	err = doGoodReqPaginated(path, db.OrderAsc, &testAccountsResponse{}, appendIter)
	require.NoError(t, err)
	assert.Greater(t, len(fetchedAccounts), 0)
	assert.LessOrEqual(t, len(fetchedAccounts), len(tc.accounts))
	...
	path = fmt.Sprintf("%s?cbEthereumAddress=%s&BJJ=%s&limit=%d", endpoint, tc.accounts[0].EthAddr, tc.accounts[0].PublicKey, limit)
	...
	// 400
	path = fmt.Sprintf("%s?cbEthereumAddress=cb:0x123456", endpoint)
	err = doBadReq("GET", path, nil, 400)
	require.NoError(t, err)
	...
	path = fmt.Sprintf("%s/%s", endpoint, "cb:UNI:258")
	account = testAccount{}
	require.Error(t, doGoodReq("GET", path, nil, &account))

	// 400
	path = fmt.Sprintf("%s/cb:12345", endpoint)
	err = doBadReq("GET", path, nil, 400)
	require.NoError(t, err)

	// 404
	path = fmt.Sprintf("%s/cb:10:12345", endpoint)
	err = doBadReq("GET", path, nil, 404)
	require.NoError(t, err)

	// 400
	path = fmt.Sprintf("%s?cb:cb:25641", endpoint)
	err = doBadReq("GET", path, nil, 400)
	require.NoError(t, err)

	// 400
	path = fmt.Sprintf("%s?cb:cb:0xb4A2333993a70fD103b7cC39883797Aa209bAa21", endpoint)
	err = doBadReq("GET", path, nil, 400)
	require.NoError(t, err)
}
