...
func (hdb *HistoryDB) GetTxAPI(txID common.TxID) (*TxAPI, error) {
	...
	err = meddler.QueryRow(
		...
		cb_idx(tx.effective_from_idx, token.symbol) AS from_idx, tx.from_eth_addr, tx.from_bjj,
		cb_idx(tx.to_idx, token.symbol) AS to_idx, tx.to_eth_addr, tx.to_bjj,
...
func (hdb *HistoryDB) GetExitAPI(batchNum *uint, idx *common.Idx) (*ExitAPI, error) {
	...
	err = meddler.QueryRow(
		...
		cb_idx(exit_tree.account_idx, token.symbol) AS account_idx,
...
func (hdb *HistoryDB) GetAccountAPI(idx common.Idx) (*AccountAPI, error) {
	...
	defer hdb.apiConnCon.Release()
	account := &AccountAPI{}
	err = meddler.QueryRow(hdb.dbRead, account, `SELECT account.item_id, cb_idx(account.idx,
...
func (hdb *HistoryDB) GetAccountsAPI(
	...
	var query string
	var args []interface{}
	queryStr := `SELECT account.item_id, cb_idx(account.idx, token.symbol) as idx, account.batch_num,
