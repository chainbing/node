...
import (
	...
	"github.com/chainbing/node/config"
	dbUtils "github.com/chainbing/node/db"
	"github.com/chainbing/node/db/historydb"
	"github.com/chainbing/node/db/kvdb"
	"github.com/chainbing/node/db/l2db"
	"github.com/chainbing/node/log"
	"github.com/chainbing/node/node"
...
func cmdGetAccountDetails(c *cli.Context) error {
	...
	for index, account := range apiAccounts {
		...
		log.Infof(" - CB Address  : %s", account.EthAddr)
...
// Config is the configuration of the chainbing node execution
type Config struct {
	mode node.Mode
	node *config.Node
}
...
func main() {
	...
	app.Commands = []*cli.Command{
		...
		},
		{
			Name:    "run",
			Aliases: []string{},
			Usage:   "Run the node in the indicated mode",
			Action:  cmdRun,
			Flags:   flags,
		},
		{
...
func checkAccountParam(c *cli.Context) (*ethCommon.Address, *babyjub.PublicKeyComp, []common.Idx, error) {
	...
	if strings.HasPrefix(accountParam, "0x") { //Check ethereum address
		addr, err = common.CbStringToEthAddr("cb:"+accountParam, "cbEthereumAddress")
		if err != nil {
			return nil, nil, nil, err
		}
	} else if len(accountParam) > characters { //Check internal chainbing account address
		bjj, err = common.CbStringToBJJ(accountParam, "BJJ")
		if err != nil {
			return nil, nil, nil, err
		}
