...
func (a *AccountCreationAuth) toHash(chainID uint16,
	...
	signerData := ethSigner.TypedData{
		...
		Domain: ethSigner.TypedDataDomain{
			...
			VerifyingContract: chainbingContractAddr.Hex(),
		},
...
func (a *AccountCreationAuth) HashToSign(chainID uint16,
	chainbingContractAddr ethCommon.Address) ([]byte, error) {
	b, err := a.toHash(chainID, chainbingContractAddr)
	if err != nil {
		return nil, tracerr.Wrap(err)
	}
...
func (a *AccountCreationAuth) Sign(signHash func(hash []byte) ([]byte, error),
	...
	hash, err := a.HashToSign(chainID, chainbingContractAddr)
	if err != nil {
...
// chainID and chainbingContractAddress passed by parameter. VerifySignature
// follows the EIP-712 encoding.
func (a *AccountCreationAuth) VerifySignature(chainID uint16,
	...
	if err != nil {
		...
		return false, fmt.Errorf("error calculating hash to be signed: %s. "+
			"ChainId: %d. ChainbingContractAddress: %s. EthereumAddress: %s. Bjj: %s. "+
			"Signature: %s. Timestamp: %s", err.Error(), chainID, chainbingContractAddr.String(),
	...
	if err != nil {
		...
		return false, fmt.Errorf("error unmarshalling public key: %s. ChainId: %d. "+
			"ChainbingContractAddress: %s. EthereumAddress: %s. Bjj: %s. Signature: %s. "+
			"Timestamp: %s", err.Error(), chainID, chainbingContractAddr.String(),
			a.EthAddr.String(), BjjToString(a.BJJ), signatureV, a.Timestamp.String())
	}
	// Get addr from pubK
	...
	if addr != a.EthAddr {
		...
		return false, fmt.Errorf("error: Ethereum address doesn't match with the one used in the signature. "+
			"ChainId: %d. ChainbingContractAddress: %s. EthereumAddress: %s. Bjj: %s. Signature: %s. Timestamp: %s",
			chainID, chainbingContractAddr.String(), a.EthAddr.String(), BjjToString(a.BJJ), signatureV, a.Timestamp.String())
	}
	return true, nil
}
