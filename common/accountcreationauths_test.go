...
func TestAccountCreationAuthSignVerify(t *testing.T) {
	...
	chainID := uint16(0)
	chainbingContractAddr := ethCommon.HexToAddress("0xc344E203a046Da13b0B4467EB7B3629D0C99F6E6")
	...
	isValid, err := a.VerifySignature(chainID, chainbingContractAddr)
	require.NoError(t, err)
...
func TestAccountCreationAuthJSComp(t *testing.T) {
	...
	tv0 := testVector{
		...
		chainID:            uint16(4),
		chainbingContractAddr: "0x7e5f4552091a69125d5dfcb7b8c2659029395bdf",
	...
	tv1 := testVector{
		...
		chainID:            uint16(0),
		chainbingContractAddr: "0x2b5ad5c4795c026514f8317c7a215e218dccd6cf",
	...
	tv2 := testVector{
		...
		chainbingContractAddr: "0xf4e77E5Da47AC3125140c470c71cBca77B5c638c",
	...
	for _, tv := range tvs {
		...
		chainID := tv.chainID
		chainbingContractAddr := ethCommon.HexToAddress(tv.chainbingContractAddr)
		a := AccountCreationAuth{
			EthAddr: ethAddr,
			BJJ:     pkComp,
		}

		toHash, err := a.toHash(chainID, chainbingContractAddr)
		require.NoError(t, err)
		assert.Equal(t, tv.toHashExpected,
			hex.EncodeToString(toHash))

		msg, err := a.HashToSign(chainID, chainbingContractAddr)
		require.NoError(t, err)
